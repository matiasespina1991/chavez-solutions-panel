...
- `id`: X-0001
- `scope`: corregir glob invalido `*.pem\` en `apps/panel/.gitignore`.
- `files`:
  - `apps/panel/.gitignore`
...
- `validation_commands`:
  - `rg -n "collection\\(|service_requests|requests-list" apps/panel/src functions/src | head`
- `status`: `approved`

## X-0002 - Alinear `firebase.json` y `apphosting*.yaml`

- `id`: X-0002
- `scope`: alinear target de hosting/apphosting al dashboard (`apps/panel`) y declarar fuente de verdad.
- `files`:
  - `firebase.json`
